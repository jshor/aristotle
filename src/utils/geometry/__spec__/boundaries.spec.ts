import { createItem } from '@/store/__tests__/__helpers__/helpers'
...
describe('Boundaries geometry utility', () => {
  ...
  describe('getItemBoundingBox()', () => {
    ...
    const baseItem = createItem('item', ItemType.OutputNode, {
      position,
      width: 20,
      height: 30
    })
    ...
    it('should return the bounding box for an item that is not rotated', () => {
      const item = { ...baseItem, rotation: 2 }

      expect(boundaries.getItemBoundingBox(item)).toEqual({
        bottom: 50,
        left: 12,
        right: 32,
        top: 20
    ...
    it('should return the bounding box for an item that is rotated 90 degrees', () => {
      const item = { ...baseItem, rotation: 1 }
    ...
    it('should return the bounding box for an item that is rotated 270 degrees', () => {
      const item = { ...baseItem, rotation: 3 }
